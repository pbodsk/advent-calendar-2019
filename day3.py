...
def calculate_path(coordinates):
    ...
    for coordinate in coordinates:
        ...
        # now we know the direction, next, store the steps in a dictionary
...
def calculate_distances(intersected_coordinates):
    distances = []
    for coordinate_set in intersected_coordinates:
        distances.append(calculate_distance(coordinate_set, (0, 0)))
...
def calculate_min_steps(intersected_coordinates, path1, path2):
    min_val = float('inf')
    for coordinate in intersected_coordinates:
        min_val = min(min_val, (path1[coordinate] + path2[coordinate]))

    return min_val


coordinates_1 = input_1.split(",")
coordinates_2 = input_2.split(",")
path1 = calculate_path(coordinates_1)
path2 = calculate_path(coordinates_2)
# now find places where the wires cross, that is, a set of intersections
set_path1 = set(path1.keys())
set_path2 = set(path2.keys())

intersection_set = set_path1 & set_path2

# And finally, find the set with the shortest distance from 0, 0
distances = calculate_distances(list(intersection_set))
print(min(distances))

print(calculate_min_steps(intersection_set, path1, path2))
